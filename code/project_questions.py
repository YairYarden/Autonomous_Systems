...
class ProjectQuestions:
    ...
    def Q1(self):
        ...
        # print("num of high err : ", num_high_err)
        # print("max err : ", max_err)
        # print("mean err : ", np.mean(err_vec))

        fig2, ax2 = plt.subplots()
        ax2.plot(err_vec)
        plt.show()
        mse_vector = np.mean(np.square(trueTrajectory[:, 0:2] - pf.history[:, 0:2]), axis=1)
        fig2, ax2 = plt.subplots()
        ax2.plot(mse_vector)
        ax2.set_title("MSE")
        ax2.set_xlabel("Frame number")
        ax2.set_ylabel("MSE [m^2]")
        ax2.grid()
        ...
        if is_save_animation:
            ...
            graphs.save_animation(pf_animation, save_path, "Particle_filter_animation_sigma_0.1")
    ...
    def Q2(self):
        ...
        print("Elapsed time FullPC: {:.4f} seconds".format(elapsed_time_kdree))
        ...
        clouds = [pc_target_f, pc_source_f]
        ...
        print("Elapsed time Filtered KNN: {:.4f} seconds".format(elapsed_time_knn))
